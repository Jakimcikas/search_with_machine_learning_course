...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        feature_results["salesRankMediumTerm"] = []
        feature_results["salesRankLongTerm"] = []
        feature_results["click_prior"] = []

        valid_keys = ["name_match", "name_phrase_match", "name_hyphens_min_df", "salePrice", "regularPrice"]

        # Fetch hits
        hits = response["hits"]["hits"]

        for hit in hits:
            log_entry_dict = hit["fields"]["_ltrlog"][0]["log_entry"]

            feature_results["doc_id"].append(hit["_id"])
            feature_results["query_id"].append(query_id)
            feature_results["sku"].append(hit["_source"]["sku"][0])

            for log in log_entry_dict:
                field, value = log["name"], log.get("value", 0)
                feature_results[field].append(value)
        ...
        return frame.astype({
                ...
                'sku': 'int64',
                'name_match':'float32',
                'name_phrase_match': 'float32',
                'name_hyphens_min_df': 'float32',
                'salePrice': 'float32',
                'regularPrice': 'float32',
                'salesRankShortTerm': 'int64'
            })
